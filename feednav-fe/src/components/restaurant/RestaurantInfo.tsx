...
const RestaurantInfo: React.FC<RestaurantInfoProps> = ({ restaurant }) => {
  ...
  const detailItems = [
    ...
    { id: 'website', icon: Globe, label: "網站", value: restaurant.website, href: restaurant.website },
    { id: 'hours', icon: Clock, label: "營業時間", value: restaurant.opening_hours },
    { id: 'cuisine', icon: Utensils, label: "菜系", value: restaurant.cuisine },
    { id: 'description', icon: Info, label: "描述", value: restaurant.description, isParagraph: true },
  ];
  ...
  return (
    <div className="space-y-6">
      {detailItems.map((item, index) => {
        ...
        return (
          <div key={index} className="flex items-start animate-fade-in" style={{ animationDelay: `${400 + index * 100}ms`, animationFillMode: 'backwards' }}>
            <item.icon className="w-4 h-4 mr-4 mt-1 text-primary shrink-0" />
            <div>
              <p className="font-semibold text-sm text-muted-foreground">{item.label}</p>
              {item.isParagraph ? (
                <p className="text-foreground/90 whitespace-pre-wrap mt-1">{item.value}</p>
              ) : item.href ? (
