...
export default function VisitedPage() {
    ...
    const { visited, removeVisited, addVisited, isMutatingVisited } = useVisitedRestaurants(session?.user?.id);
    const { favorites, addFavorite, removeFavorite, isMutating: isMutatingFavorite } = useFavorites(session?.user?.id);
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
            <Header
              ...
              sortBy="default"
              setSortBy={() => {}}
              filters={{ district: "all", cuisine: "all", priceRange: [1, 4], tags: [], showOpenOnly: false }}
              setFilters={() => {}}
              handleRandomSelect={() => {}}
            />
            <main className="container py-8">
                <h1 className="text-3xl font-bold tracking-tight mb-6">我的美食足跡 ({visitedRestaurants.length})</h1>
