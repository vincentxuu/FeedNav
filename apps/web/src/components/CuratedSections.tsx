...
const CuratedSection = ({ title, icon, restaurants, ...rest }: CuratedSectionProps) => (
  <div className="space-y-4">
    ...
    {restaurants.length > 0 ? (
      <div className="-mx-4 grid auto-cols-[260px] grid-flow-col gap-4 overflow-x-auto px-4 pb-4">
        {restaurants.map((r, i) => (
...
export const CuratedSections = ({
  ...
  const hotRestaurants = useMemo(() => {
    ...
    const favoriteCounts: Record<number, number> = favorites.reduce(
      (acc, fav) => {
    ...
    return [...enrichedRestaurants]
      .sort(
        (a, b) =>
          (favoriteCounts[parseInt(b.id, 10)] || 0) - (favoriteCounts[parseInt(a.id, 10)] || 0)
      )
      .slice(0, 10)
